...
window.GLOSSARY = {
  ...
  elevatorapi:{abbr:"机器人电梯互联接口", en:"Robot–Elevator Integration API", zh:"机器人-电梯互联接口",
    ...
    source:null},
  servoing:{abbr:"视觉伺服", en:"Visual Servoing", zh:"视觉伺服",
    def:"用视觉反馈闭环驱动机器人运动：不是“看一眼算出目标再盲走过去”，而是每一帧都比较当前图像与目标的差异，直接算出该往哪动，误差随迭代收敛到零。分 IBVS（误差定义在图像平面，对标定不敏感）与 PBVS（先重建三维位姿再控制，依赖标定精度）。它是补“最后几厘米”的经典答案——手眼标定再准也会漂移，而视觉伺服把标定误差变成能被闭环吃掉的量。经典实现 ViSP 是 GPL-2.0，且官方教程只覆盖定臂，接到浮动基座人形上还得自己补雅可比。",
    source:null},
  handeye:{abbr:"手眼标定", en:"Hand-Eye Calibration", zh:"手眼标定",
    def:"求解相机坐标系与机器人坐标系之间的固定变换，经典形式是解 AX = XB。没有它，视觉算出的“目标在这儿”翻译不成“手该去哪儿”。分 eye-in-hand（相机装在手上）与 eye-to-hand（相机固定在外看机器人），开源工具往往只支持其中一种。人形的特殊困难在于：串联臂基座固定、标一次能用很久，而人形站立时基座本身在晃，误差随姿态漂移——所以通常必须再叠一层视觉伺服。",
    source:null},
  ovd:{abbr:"开放词汇检测", en:"Open-Vocabulary Detection", zh:"开放词汇检测",
    def:"用自然语言指定要找的东西，不必事先为它训练类别——输入“写着 3 的圆形按钮”就直接框出来。代表作 GroundingDINO、Grounded-SAM、OWLv2、YOLO-World。对长尾小众目标意义重大：电梯按钮正是“类别多、每类样本极少”的典型（公开数据集 368 类平均每类约 3.8 张），而这条路线根本不用训。代价是小目标、反光面板上的召回没人系统评测过。",
    source:{url:"https://arxiv.org/abs/2303.05499", label:"代表作 · GroundingDINO, Liu et al. 2023"}},
  contactrich:{abbr:"接触丰富操作", en:"Contact-Rich Manipulation", zh:"接触丰富操作",
    def:"成败由接触事件而非自由空间轨迹决定的操作：插孔、装配、拧螺丝、按按钮。共同难点是接触部分可观测——相机看不见指尖底下发生了什么。常见对策有三类：力/阻抗顺应、把接触写进仿真判据（RoboCasa 用接触+撤手，Meta-World 用行程阈值）、训练期加装传感（在指尖装麦克风采集接触音频，部署时不依赖）。按按钮是其中最小的一个：力只有几牛顿，但“碰到”与“按下去”之间差着 2–3 mm 弹簧行程，且这段行程在图像上几乎不可见。",
    source:null},
